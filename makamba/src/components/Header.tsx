        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <nav className="hidden md:flex space-x-8">
              {navigationItems.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
                  className="text-gray-700 hover:text-[#FF6700] px-3 py-2 text-sm font-medium transition-colors duration-200 dark:text-white rounded-lg" 
                >
                  {item.name}
                </a>
              ))}
            ...
            <div className="hidden md:flex items-center space-x-4">
               <ModeToggle />
              <Button className='dark:bg-white dark:text-black bg-[#FF6700] text-white hover:bg-[#CC191B] hover:duration-300'>Contacto</Button>
            </div>
      ...
      <div className={`fixed inset-0 z-50 md:hidden transition-opacity duration-300 ${
        ...
        <div className={`absolute left-0 top-0 h-full w-80 max-w-[80vw] bg-white shadow-xl transform transition-transform duration-300 ease-out ${
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <div className="flex items-center">
              <img src={Logo} alt="Elitebuilders Logo" className="h-8 w-auto" />
            </div>
            <button
              onClick={toggleMobileMenu}
              className="p-2 text-gray-500 hover:text-black transition-colors duration-200"
          ...
          <div className="px-6 py-6">
            <nav className="space-y-2">
              {navigationItems.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
                  className="block px-4 py-3 text-gray-700 hover:text-[#FF6700] hover:bg-gray-50 rounded-lg text-base font-medium transition-all duration-200"
                  onClick={toggleMobileMenu}
                >
                  {item.name}
                </a>
            ...
            {/* Contact Button */}
            <div className="mt-8">
              <Button className="w-full dark:bg-white dark:text-black bg-[#FF6700] text-white hover:bg-[#CC191B] hover:duration-300 transition-colors duration-200">
                Contacto
              </Button>
            </div>
          </div>
