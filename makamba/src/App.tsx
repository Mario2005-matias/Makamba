...
const SplashCursor = () => {
  useEffect(() => {
    ...
    cursor.style.cssText = `
      ...
      pointer-events: none;
      z-index: 9999;
      mix-blend-mode: ${isMobile ? 'screen' : isTablet ? 'overlay' : 'multiply'};
    ...
    style.textContent = `
      ...
      .splash-cursor.clicking {
        transform: ${isMobile ? 'scale(2)' : isTablet ? 'scale(1.8)' : 'scale(1.5)'};
        background: ${isMobile ? 
    ...
    const handleMouseDown = (e) => {
      ...
      const rippleAnimation = isMobile ? 'mobileRipple' : isTablet ? 'tabletRipple' : 'ripple';
    ...
    const handleMouseEnter = (e) => {
      if (e.target.matches('a, button, [role="button"]')) {
        ...
        if (isMobile) {
          trail.style.background = 'radial-gradient(circle, rgba(255, 140, 66, 0.3), #FF6700, transparent)';
        }
      }
    };
    ...
    const handleMouseLeave = (e) => {
      if (e.target.matches('a, button, [role="button"]')) {
        ...
        if (isMobile) {
          trail.style.background = 'radial-gradient(circle, transparent, #FF6700, transparent)';
        }
      }
    };
